...
class BlogUpdateView(LoginRequiredMixin, UpdateView):
    ...
    fields = ["title", "description", "thumbnail",'image_url']
...
class BlogDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        pk=self.kwargs['pk']
        return self.model.objects.filter(author=self.request.user).filter(pk=pk)
...
class Login(LoginView):
    ...
    def form_valid(self, form):
        ...
        #             pass
        # else:
        #     messages.success(self.request,'Please enter valid email')
...
class AccountVerify(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        token=self.kwargs['auth_token']
        print(token)
        user = User.objects.filter(email_verification_token=kwargs['auth_token']).first()
        print(user)
        if not user:
            return HttpResponse("<h3>Sorry ,token invalid</h3>")
